...
module Streaming.Events
  ( -- * Event Types
    SSEEvent (SSERequestStarted, SSERequestCompleted, SSEProofGenerated, SSEProviderStatus, SSEModelNew, SSEMetricsUpdate, SSEKeepalive),
    EventType (RequestStarted, RequestCompleted, ProofGenerated, ProviderStatus, MetricsUpdate, ModelNew, Keepalive),
    ...
    ModelNewData (ModelNewData, mndModelId, mndProvider, mndContextWindow, mndMaxOutput, mndModality, mndSupportsTools, mndSupportsVision, mndFamily, mndTimestamp),
...
-- | Data for model.new event (new model detected)
data ModelNewData = ModelNewData
  { mndModelId :: !Text,
    mndProvider :: !Text,
    mndContextWindow :: !(Maybe Int),
    mndMaxOutput :: !(Maybe Int),
    mndModality :: !Text, -- "text", "vision", "multimodal"
    mndSupportsTools :: !Bool,
    mndSupportsVision :: !Bool,
    mndFamily :: !(Maybe Text), -- "llama", "claude", "gpt", etc
    mndTimestamp :: !Text
  }
  deriving (Eq, Show)

instance ToJSON ModelNewData where
  toJSON ModelNewData {..} =
    object
      [ "model_id" .= mndModelId,
        "provider" .= mndProvider,
        "context_window" .= mndContextWindow,
        "max_output" .= mndMaxOutput,
        "modality" .= mndModality,
        "supports_tools" .= mndSupportsTools,
        "supports_vision" .= mndSupportsVision,
        "family" .= mndFamily,
        "timestamp" .= mndTimestamp
      ]
...
data SSEEvent
  ...
  | SSEModelNew ModelNewData
  | SSEMetricsUpdate LBS.ByteString -- Pre-encoded metrics JSON
  | SSEKeepalive
  deriving (Eq, Show)

instance ToJSON SSEEvent where
  toJSON (SSERequestStarted d) =
    object
      [ "type" .= RequestStarted,
        "data" .= d
      ]
  toJSON (SSERequestCompleted d) =
    object
      [ "type" .= RequestCompleted,
        "data" .= d
      ]
  toJSON (SSEProofGenerated d) =
    object
      [ "type" .= ProofGenerated,
        "data" .= d
      ]
  toJSON (SSEProviderStatus d) =
    object
      [ "type" .= ProviderStatus,
        "data" .= d
      ]
  toJSON (SSEModelNew d) =
    object
      [ "type" .= ModelNew,
        "data" .= d
      ]
  toJSON (SSEMetricsUpdate _) =
    object
      [ "type" .= MetricsUpdate
...
-- | Emit new model detected event
emitModelNew :: EventBroadcaster -> Text -> Text -> Maybe Int -> Maybe Int -> Text -> Bool -> Bool -> Maybe Text -> Text -> IO ()
emitModelNew bc modelId provider contextWindow maxOutput modality supportsTools supportsVision family timestamp =
  broadcast bc $
    SSEModelNew
      ModelNewData
        { mndModelId = modelId,
          mndProvider = provider,
          mndContextWindow = contextWindow,
          mndMaxOutput = maxOutput,
          mndModality = modality,
          mndSupportsTools = supportsTools,
          mndSupportsVision = supportsVision,
          mndFamily = family,
          mndTimestamp = timestamp
        }
