...
chatCompletion configRef ctx req = G.do
  recordAuthUsage "fireworks" "api-key"
  config <- liftIO' $ readIORef configRef
...
listModels _ _ = G.do liftIO' $ pure $ Success $ ModelList "list" $ map (\mid -> Model (ModelId mid) "model" (Timestamp 0) "fireworks") fireworksModels
...
handleErrorStatus status body = case status of
  401 -> Failure $ AuthError "Invalid Fireworks API key"
  429 -> Retry $ RateLimitError "Fireworks rate limit"
  500 -> Retry $ InternalError $ "Fireworks 500: " <> bodyText
  502 -> Retry $ ProviderUnavailable "Fireworks 502"
  503 -> Retry $ ProviderUnavailable "Fireworks 503"
  504 -> Retry $ TimeoutError "Fireworks 504"
  _ -> Failure $ InvalidRequestError $ "Fireworks " <> T.pack (show status) <> ": " <> bodyText
  where
