...
makeAnthropicProvider configRef = Provider
...
anthropicChatStream manager apiKey baseUrl req onDelta = do
    ...
    result <- withLatency $ makeStreamingRequestWithParsing 
        manager url apiKey (encode streamReq)
        onDelta toolCallsRef stopReasonRef usageRef
...
fromAnthropicResponse A.ChatResponse{..} = OpenAI.ChatResponse
  ...
  where
    choice = OpenAI.Choice
        ...
        , OpenAI.choiceMessage = OpenAI.Message
...
makeStreamingRequestWithParsing 
    ...
    -> LBS.ByteString 
    -> (Text -> IO ())          -- ^ Content delta handler
    -> IORef [A.ToolUse]        -- ^ Accumulated tool calls
