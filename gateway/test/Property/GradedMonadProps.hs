...
prop_gatewayMLeftIdentity = withTests 10 . property $ do
  ...
  let m1 = liftPure x >>= \y -> liftPure (y + 1) :: GatewayM Pure Int
      m2 = liftPure (x + 1) :: GatewayM Pure Int
...
prop_gatewayMRightIdentity = withTests 10 . property $ do
  x <- forAll genInt
  let m = liftPure x :: GatewayM Pure Int
      m' = m >>= liftPure
...
prop_gatewayMAssociativity = withTests 10 . property $ do
  x <- forAll genInt
  let m = liftPure x :: GatewayM Pure Int
      f :: Int -> GatewayM Pure Int
      f y = liftPure (y + 1)
      g :: Int -> GatewayM Pure Int
      g y = liftPure (y * 2)
      left = (m >>= f) >>= g
      right = m >>= (\y -> f y >>= g)
