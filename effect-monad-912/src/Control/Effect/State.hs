...
instance {-# OVERLAPPING #-} Show (Var "z") where
    show Z   = "z"
    show Var = "Var"
instance {-# OVERLAPPABLE #-} Show (Var v) where
    show _ = "Var"
...
instance {-# OVERLAPPING #-} Update ((v :-> a :! R) ': as) as' => Update ((v :-> a :! W) ': (v :-> b :! R) ': as) as' where
    update (Ext (v :-> (a :! _)) (Ext _ xs)) = update (Ext (v :-> (a :! (Eff::(Action R)))) xs)

instance {-# OVERLAPPABLE #-} Update ((u :-> b :! s) ': as) as' => Update ((v :-> a :! W) ': (u :-> b :! s) ': as) as' where
    update (Ext _ (Ext e xs)) = update (Ext e xs)

instance {-# OVERLAPPABLE #-} Update ((u :-> b :! s) ': as) as' => Update ((v :-> a :! R) ': (u :-> b :! s) ': as) ((v :-> a :! R) ': as') where
