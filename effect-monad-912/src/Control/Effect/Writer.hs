...
instance Effect Writer where
    ...
    {-| Composing computations takes the union of the writer states, using the monoid
...
{-| Define the operation for removing duplicates using (<>) -}
instance (Semigroup u, Nubable ((k :-> u) ': s)) => Nubable ((k :-> u) ': (k :-> u) ': s) where
    nub (Ext _ u (Ext k v s)) = nub (Ext k (u <> v) s)
